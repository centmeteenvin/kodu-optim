...
class OrchestratorConfig:
    ...
    def __init__(self, path: Path | None = None):
        if path is None:
            ...
            with path.open("r") as file:
                ...
                self.data_dir = pathlib.Path(config.get("data_dir", './data')).absolute()
                
        self.data_dir.mkdir(parents=True, exist_ok=True)
        self.db_url = f"postgresql://{self.db_user}:{self.db_password}@{self.db_host}:{self.db_port}/"
        OrchestratorConfig._active = self

    def __str__(self):
        return (f"OrchestratorConfig(host={self.host}, port={self.port}, "
    ...
    def __repr__(self):
        return (f"OrchestratorConfig(host={self.host!r}, port={self.port!r}, "
